...
def generate_access_token(access_token_url, client_id, client_secret, code):
    """
    create an access token to social Oauth2.
    :param client_id: client_id from provider such as https://github.com/settings/developers
    :param client_secret: client_secret from provider such as https://github.com/settings/developers
    :param code: code generated by client from provider such as http://github.com/login/oauth/authorize/
    ...
    token = re.search(
        r"access_token=([a-zA-Z0-9]+)", auth_response.content.decode('utf-8'))
    if token is None:
